...
[source,bash]
----
export GRAFANA_URL=https://$(oc get route grafana -n istio-system --template='{{ .spec.host }}')
echo $GRAFANA_URL
----
...
[source,bash]
----
export JAEGER_URL=https://$(oc get routes -n istio-system jaeger --template='{{ .spec.host }}')
echo $JAEGER_URL
----
...
Kiali username: devXX
Kiali password: devXX
